...
export async function getChapters(): Promise<Chapter[]> {
  ...
  if (!res.ok) throw new Error(`Failed to fetch chapters (status ${res.status})`);
...
export async function getChapterDetails(chapterNum: number): Promise<Chapter> {
  ...
  if (!res.ok) throw new Error(`Failed to fetch chapter ${chapterNum} (status ${res.status})`);
  return await res.json();
}
...
export async function getVerseDetails(chapterNum: number, verseNum: number): Promise<Verse> {
  ...
  if (!res.ok) throw new Error(`Failed to fetch verse ${chapterNum}.${verseNum} (status ${res.status})`);
  return await res.json();
}
